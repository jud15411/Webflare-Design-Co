...
const Orchestrator = ({ user }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-400 mx-auto">
      {/* Header Section */}
      ...
      {/* Tab Content */}
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-[2.5rem] overflow-hidden shadow-sm transition-colors duration-300">
        {/* Personnel Tab */}
        {activeTab === 'users' && (
                  ...
                  <tr
                    ...
                    <td className="p-6">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center font-black text-white text-xs shrink-0">
        ...
        {activeTab === 'roles' && (
          <div className="flex flex-col md:flex-row h-[calc(100vh-320px)] min-h-125">
            {/* Sidebar with Role Search */}
            <div className="w-full md:w-72 border-b md:border-b-0 md:border-r border-slate-100 dark:border-zinc-800 flex flex-col bg-slate-50/50 dark:bg-zinc-950/30">
              <div className="p-6 border-b border-slate-100 dark:border-zinc-800 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="w-6 h-6 rounded-lg bg-indigo-600 text-white flex items-center justify-center font-black hover:bg-indigo-700 transition-shadow shadow-lg shadow-indigo-500/20">
                    +
                  </button>
                </div>
                <input
                  ...
                  className="w-full bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl px-4 py-2 text-[10px] font-bold text-slate-700 dark:text-zinc-100 outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all placeholder:text-slate-400"
                />
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                {filteredRoles.map((role) => (
                  <button
                    ...
                    className={`w-full flex flex-col items-start p-4 rounded-2xl transition-all border ${
                    ...
                    <span className="text-[8px] font-bold opacity-40 dark:opacity-60 uppercase mt-1">
                      Branch: {role.branch}
                    </span>
                  </button>
                ))}
            ...
            {/* Main Content with Permission Search */}
            <div className="flex-1 flex flex-col bg-white dark:bg-zinc-900 overflow-hidden">
              {activeRole ? (
                <>
                  <div className="p-8 border-b border-slate-50 dark:border-zinc-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                    ...
                    <input
                      ...
                      className="w-full md:w-64 bg-slate-50 dark:bg-zinc-950/50 border border-slate-100 dark:border-zinc-800 rounded-xl px-4 py-3 text-[10px] font-bold text-slate-700 dark:text-zinc-100 outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all"
                    />
                  </div>
      ...
      {showRoleModal && (
        <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-zinc-950 w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl border border-white/10 dark:border-zinc-800">
            ...
            <form onSubmit={handleCreateRole} className="space-y-6">
              <div className="space-y-2">
                ...
                <input
                  type="text"
                  required
                  className="w-full bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-4 text-sm font-bold text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
              ...
              {isAdmin && (
                <div className="space-y-2">
                  ...
                  <select
                    className="w-full bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-4 text-sm font-bold text-slate-900 dark:text-white outline-none"
              ...
              <div className="flex gap-4 pt-4">
                <button
                  ...
                  className="flex-1 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-zinc-500 hover:bg-slate-100 dark:hover:bg-zinc-900 transition-all">
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-indigo-500/20 transition-all">
                  Initialize
                </button>
              </div>
            </form>
