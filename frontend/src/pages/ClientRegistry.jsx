...
const ClientRegistry = ({ user }) => {
  ...
  // 1. Function defined at the top level so it is accessible everywhere
  ...
  // Can they see the deep-dive console?
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-black text-slate-900 dark:text-indigo-500 uppercase tracking-tighter">
            Client Registry
          </h1>
        ...
        {canRegister && (
          <button
            ...
            className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl font-black uppercase text-xs transition-all shadow-xl shadow-indigo-500/20 active:scale-95">
            + Register Client
          </button>
        )}
      </div>
        ...
        {loading ? (
                ...
                <tr
                  ...
                  <td className="p-6">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-zinc-800 flex items-center justify-center font-black text-indigo-500 border border-slate-200 dark:border-zinc-700">
                        {client.logoUrl ? (
                          <img
                            src={client.logoUrl}
                            alt=""
                  ...
                  <td className="p-6 text-sm font-medium text-slate-500 dark:text-zinc-400">
                    {client.industry}
                  </td>
                  <td className="p-6">
                    <span
                      className={`px-3 py-1 text-[10px] font-black uppercase rounded-lg border ${
                  ...
                  <td className="p-6 text-right">
                    {canViewConsole ? (
                      <button
                        ...
                        className="px-4 py-2 text-[10px] font-black text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-xl transition-all uppercase tracking-tighter">
                        View Console →
                      </button>
                    ) : (
          ...
          <div className="p-20 text-center">
            ...
            {user.branch === 'admin' && (
              <button
                onClick={() => setIsModalOpen(true)}
                className="mt-4 text-indigo-500 font-black uppercase text-[10px] hover:underline">
      ...
      {/* 3. The Modal receives the fetchClients function to refresh the list after a POST */}
      <ClientModal
        isOpen={isModalOpen}
